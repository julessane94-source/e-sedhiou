...
@section('content')
    ...
    <section class="hero tw:pb-8">
        <div class="hero__panel">
            ...
            <div class="tw:overflow-hidden tw:h-16 tw:flex tw:items-center">
                <h1 class="tw:font-semibold tw:text-3xl slide-animate">Registre des citoyens</h1>
            </div>
            ...
            <div class="tw:flex tw:gap-4 tw:mt-4">
                <div class="tw:px-4 tw:py-3 tw:bg-emerald-50 tw:border-l-4 tw:border-emerald-600 tw:rounded">
                    ...
                    <p class="tw:font-bold tw:text-emerald-700 tw:text-lg">{{ $availableCount }} / {{ $totalRegistrations }}</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <div class="panel tw:mb-6">
        <div class="panel-header tw:mb-6">
            <div class="panel-header__title">
                ...
                <div class="tw:overflow-hidden tw:h-10 tw:flex tw:items-center">
                    <h2 class="tw:font-semibold tw:text-xl slide-animate">Outils de consultation</h2>
                </div>
            </div>
        </div>

        <form method="get" action="{{ route('portal.admin.citizen_registry') }}" class="tw:grid tw:grid-cols-1 md:tw:grid-cols-4 tw:gap-4 tw:p-4 tw:bg-gray-50 tw:rounded-lg">
            <div class="field">
            ...
            <div class="field">
                ...
                <select id="f-sector" 
                    ...
                    @foreach ($sectors as $sector)
                        <option value="{{ $sector }}" @selected($filterSector === $sector)>{{ $sector }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="field">
                ...
                <select id="f-available" 
                    ...
                    <option value="1" @selected($filterAvailable === '1')>✅ Disponible</option>
                    <option value="0" @selected($filterAvailable === '0')>❌ Non disponible</option>
                </select>
            </div>
            ...
            <div class="tw:flex tw:gap-2 tw:items-end">
                ...
                <a class="button button--ghost tw:flex-1 tw:text-center" href="{{ route('portal.admin.citizen_registry') }}">
                    ↻ Réinit.
                </a>
            </div>
        </form>
    ...
    <div class="panel">
        <div class="panel-header tw:mb-4">
            <div class="panel-header__title">
                ...
                <div class="tw:overflow-hidden tw:h-8 tw:flex tw:items-center">
                    <h2 class="tw:font-semibold tw:text-lg slide-animate">Liste des citoyens enregistrés</h2>
                </div>
            </div>
        </div>
                        ...
                        <tr class="hover:tw:bg-gray-50 tw:transition-colors">
                            <td class="tw:px-4 tw:py-4">
                                ...
                                <div class="tw:text-xs tw:text-gray-500 tw:mt-1">
                                    📋 #{{ str_pad($registration->register_number, 6, '0', STR_PAD_LEFT) }}
                                </div>
                            </td>
                            <td class="tw:px-4 tw:py-4">
                            ...
                            <td class="tw:px-4 tw:py-4">
                                <div class="tw:font-medium tw:text-gray-900">{{ $registration->profession_title }}</div>
                                <div class="tw:text-xs tw:text-gray-500 tw:mt-1">{{ $registration->profession_sector }}</div>
                            </td>
                            <td class="tw:px-4 tw:py-4 tw:text-center">
                                <span class="tw:inline-block tw:px-3 tw:py-1 tw:bg-blue-100 tw:text-blue-800 tw:rounded-full tw:text-xs tw:font-semibold">
                                    {{ $registration->years_experience }} ans
                                </span>
                            </td>
                            <td class="tw:px-4 tw:py-4 tw:text-center">
                                @if ($registration->available_for_municipality)
                            ...
                            <td class="tw:px-4 tw:py-4 tw:text-center">
                                @if ($registration->cv_file_path)
                                    <a href="{{ route('portal.admin.citizen_registry.download_cv', $registration) }}" 
                            ...
                            <td class="tw:px-4 tw:py-4 tw:text-center tw:text-xs tw:text-gray-600">
                                ...
                                <span class="tw:text-gray-500">{{ $registration->created_at->format('H:i') }}</span>
                            </td>
                            <td class="tw:px-4 tw:py-4 tw:text-center">
                                <button type="button" 
                                        ...
                                        onclick="openDetailModal({{ $registration->id }}, `{{ addslashes($registration->first_name) }} {{ addslashes($registration->last_name) }}`)">
                                    👁️ Voir
                                </button>
                            </td>
                        </tr>
    ...
    <script>
        function openDetailModal(id, name) {
            ...
            const registration = @json($registrations->items());
